...
class CifarResNeXt(nn.Module):
    ...
    """
    def __init__(self, cardinality, depth, num_classes, Max, Min, widen_factor=4, dropRate=0):
        """ Constructor
        Args:
    ...
    def intermediate_forward(self, x, layer_index):
        ...
        if layer_index == 0:
            ...
            out = self.stage_1.forward(out)
        elif layer_index == 1:
            out = F.relu(self.bn_1.forward(out), inplace=True)
            out = self.stage_1.forward(out)
            out = self.stage_2.forward(out)
        elif layer_index == 2:
            out = F.relu(self.bn_1.forward(out), inplace=True)
            out = self.stage_1.forward(out)
            out = self.stage_2.forward(out)
            out = self.stage_3.forward(out)
        elif layer_index == 3:
            out = F.relu(self.bn_1.forward(out), inplace=True)
            out = self.stage_1.forward(out)
            out = self.stage_2.forward(out)
    ...
    def list_forward(self, x):
        ...
        x4 = F.avg_pool2d(x3, 8, 1)
        x4 = x4.view(-1, 1024)
        x4 = self.classifier(x4)
        
        return [x1.detach(),x2.detach(),x3.detach(),x4.detach()]
